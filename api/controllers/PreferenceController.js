...
module.exports = {
  ...
  modifyCategoryPreferences: function (req, res) {
    ...
    var columNumber = parseInt(sails.config.serviceConfig.columNumber, 10);
    var category = req.param('category');
    // sails.log(category);
    var userName = req.param('userName');
    // sails.log(userName);
  ...
  getUserPreferencesByCategory: function (req, res) {
      ...
      .then(function (user) {
        if (user) {
          if (user.preferences) {
            ...
            return [user, Promise.map(result, getLinks)];
          } else {
            return [user, []];
          }
        } else {
  ...
  clearPreferences: function (req, res) {
    sails.log('clearPreferences');
    var userName = req.param("userName");
    User.findOne({username: userName})
      .then(function (user) {
        if (user) {
          Preference.destroy({owner: user.id})
            .then(function (preferences) {
              sails.log('Preferences is empty.');
              return res.json({success: true, msg: 'preference 已经清空'});
            });
        } else {
          return res.json({success: false, msg: '无此用户'});
        }
      })
      .fail(function (err) {
        sails.log(err);
  ...
  initPreferences: function (req, res) {
    sails.log('initPreferences');
    var userName = req.param("userName");
    User.findOne({username: userName})
      .then(function (user) {
        if (user) {
          Preference.find({owner: user.id})
            .then(function (preferences) {
              if (preferences) {
                ExcelService.initDefaultPreference(userName);
                return res.json({success: true, msg: '初始化成功'});
              } else {
                sails.log('Preferences is not empty.');
                return res.json({success: false, msg: '请先清空preference'});
              }
            });
        } else {
          return res.json({success: false, msg: '无此用户'});
