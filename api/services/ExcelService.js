    ...
    if (category.category == entry.category) {
      Link.findOrCreate({title: entry.title, url: entry.url})
        .exec(function (err, findOrCreateLink) {
          ...
          Preference.findOrCreate({
            owner: user.id,
            category: entry.category,
            name: entry.name,
            xposition: parseInt(entry.xposition, 10),
            yposition: parseInt(entry.yposition, 10),
            link: findOrCreateLink.id
          })
...
module.exports = {
  initCatalog: function () {
    var fileName = path.resolve(sails.config.appPath, 'assets', 'initConfigFile', sails.config.serviceConfig.catalogFilePath);;
  ...
  initDefaultPreference: function (userName) {
    User.findOrCreate({username: userName})
      .then(function (user) {
        ...
        var filename = path.resolve(sails.config.appPath, 'assets', 'initConfigFile', sails.config.serviceConfig.preferenceFilePath)
        ExcelService.processExcel(filename, processor);
      })
  },
  copyFiles: function (soure, dest) {
    var soure = path.resolve(sails.config.appPath, 'assets', 'initConfigFile', sails.config.serviceConfig.preferenceFilePath);
    var dest = path.resolve(sails.config.appPath, 'assets', 'uploadFiles', sails.config.serviceConfig.preferenceFilePath);
    var readStream = fs.createReadStream(soure);
    var writeStream = fs.createWriteStream(dest);
    readStream.pipe(writeStream);
    console.log("拷贝完成:" + sails.config.serviceConfig.preferenceFilePath);
    soure = path.resolve(sails.config.appPath, 'assets', 'initConfigFile', sails.config.serviceConfig.catalogFilePath);
    dest = path.resolve(sails.config.appPath, 'assets', 'uploadFiles', sails.config.serviceConfig.catalogFilePath);
    readStream = fs.createReadStream(soure);
    writeStream = fs.createWriteStream(dest);
    readStream.pipe(writeStream);
    console.log("拷贝完成:" + sails.config.serviceConfig.catalogFilePath)
  }
}
